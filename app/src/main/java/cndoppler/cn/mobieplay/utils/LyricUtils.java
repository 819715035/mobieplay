...
public class LyricUtils
{
     ...
     * @param readLine
     */
    private String  parsedLyric(String readLine)
    {
        ////indexOf第一次出现[的位置
        int pos1 = readLine.indexOf("[");//0,如果没有返回-1
        int pos2 = readLine.indexOf("]");//9,如果没有返回-1
        if (pos1 == 0 && pos2!=-1){
            //肯定是由一句歌词
            //装时间
            long[] times = new long[getCountTag(readLine)];
            String strTime = readLine.substring(pos1+1,pos2);
            //把时间转出时间戳
